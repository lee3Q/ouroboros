...
def test_normalize_execution_acceptance_drops_auto_report_criteria() -> None:
    ...
    assert tuple(ac_text(ac) for ac in normalized.acceptance_criteria) == (
        _SINGLE_HELLO_AUTO_OBSERVATION_AC,
    )
...
def test_normalize_execution_acceptance_preserves_distinct_autoresearch_user_ac() -> None:
    ...
    assert "train.py must preserve the existing --device CLI flag behavior." in tuple(
        ac_text(ac) for ac in normalized.acceptance_criteria
    )
    assert "Final report must include the baseline val_bpb and memory." in tuple(
        ac_text(ac) for ac in normalized.acceptance_criteria
    )
